...
export class omf {
    ...
    static async forEachAsync<dataType, keyEnum = string>(
        oMap: omap<dataType, keyEnum> | undefined,
        forEachCallback: (object: dataType, key: string) => Promise<any>
    ) {
        if (!oMap) return undefined
        let a = omf.toArray(oMap)

        for (let object of a.objectsKeys) {
            await forEachCallback(object.object, object.key)

        }

        /*   await MrkLib.forEachInObject<dataType, keyEnum>(oMap, (object, key) => {
               // @ts-ignore
               await forEachCallback(object, key)
           })*/
    }
